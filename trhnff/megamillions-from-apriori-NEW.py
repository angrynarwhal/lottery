...
# calculate the avg for the old numbers and the new numbers separately
probability_list = list(number_probabilities.items())
group_one_avg = group_two_avg = 0
for item in probability_list:
    if item[0] <= 56:
        group_one_avg += item[1]
    else:
        group_two_avg += item[1]
group_one_avg /= 56
group_two_avg /= 19

# adjust the new powerball numbers accordingly
for i in range(56, 74):
    item = list(probability_list[i])
    rate = item[1]
    rate /= group_two_avg
    rate *= group_one_avg
    item[1] = rate
    probability_list[i] = tuple(item)

# test print the new numbers
for item in probability_list:
    print(item)

...
#date when numbers 57-75 were added
introduction_date = pd.to_datetime("10-22-13", format='%m-%d-%y')
total_draws = data['date'].count()
print(total_draws)
draws_before_intro = data[data['date'] < introduction_date]['date'].count()
print(draws_before_intro)
inputstatus = input("Would you like to calculate\n(1)total historical probability\n(2)historical probability over the past year\n")
...
for _, row in data.iterrows():
    ...
    for column in number_columns:
        ...
        if(int(inputstatus) == 1):
            ...
            if pd.notnull(number):
                number = int(number)
                if 1 <= number <= max_number and date >= introduction_date:  # Check if the number is within the valid range
                    number_frequency[number] = number_frequency.get(number, 0) + 1
...
if(int(inputstatus) == 1):
    ...
    for number,freq in number_frequency.items():
        ...
        if(number >=57):
            number_probabilities[number] = freq / (total_draws-draws_before_intro)
        else:
            number_probabilities[number] = freq / total_draws
else:

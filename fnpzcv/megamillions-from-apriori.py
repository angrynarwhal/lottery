...
data = pd.read_csv(os.path.join('..','megamillions.csv'), parse_dates=['date'], date_format='%m-%d-%y')
...
# ...
# Calculate the number of draws from beginning of data to balls 57-75 introduced
draws_with_old_balls = len(data[data['date'] < '2013-10-22'])

# Calculate the number of draws from introduction of balls 57-75 to present
draws_with_new_balls = len(data[data['date'] >= '2013-10-22'])

# Calculate normalization factor
nf = draws_with_new_balls / draws_with_old_balls

# Weighted averages to apply 
weight_before_new_balls = 56/(56+75)
weight_after_new_balls = 75/(56+75)

# Normalize the data
for number in range(1, max_number + 1):
    if number in number_probabilities:
        if data['date'].min() >= pd.Timestamp('2013-10-22'):
            # Check if the date range falls after new balls introduced
            number_probabilities[number] = (number_probabilities[number] * weight_after_new_balls) / nf
        else:
            number_probabilities[number] = (number_probabilities[number] * weight_before_new_balls) / nf

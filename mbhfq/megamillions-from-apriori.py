...
for _, row in data.iterrows():
    ...
    # normalizes the data so that it is only looking at the times when the numbers 57-75 were in the data
    if date < pd.to_datetime("1/1/2014") or date > pd.to_datetime("1/1/2018"):
        continue
    # code to normalize the data so that it is only looking at the current possible lottery numbers (1-70) that can be drawn

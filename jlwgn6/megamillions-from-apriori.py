...
#mapping each number to how many years the number has been active 
def map_number_to_years_active(number):
    if ( 1 <= number <= 56 ):
        return 21
    elif (57 <= number <= 70):
        return 9
...
# keeps track of new normalized value total 
total = 0

for number in number_frequency:
    # normalizing by dividing the frequency by the years active 
    number_frequency[number] = math.floor(number_frequency[number]*100/map_number_to_years_active(number))
    total = total + number_frequency[number]

# changing numbers to percentages 
for number in number_frequency:
    number_frequency[number] = number_frequency[number]/total 
...
# changed freq/total_draws_1_year from division to multiplication 

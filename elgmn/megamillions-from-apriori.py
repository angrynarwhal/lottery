...
data = pd.read_csv(os.path.join(os.path.dirname(__file__), os.pardir, 'megamillions.csv'), parse_dates=['date'], date_format='%m-%d-%y')
...
scaling_factor = 8640000000
range_01_56 = (datetime.date(2023, 7, 28) - datetime.date(2002, 5, 17)).total_seconds() / scaling_factor
range_57_70 = (datetime.date(2023, 7, 28) - datetime.date(2013, 10, 22)).total_seconds() / scaling_factor
range_70_75 = (datetime.date(2017, 10, 27) - datetime.date(2013, 10, 22)).total_seconds() / scaling_factor

# Normalizes frequencies based on when numbers were possible to be drawn
for key, value in number_frequency.items():
    if 1 <= key <= 56:
        number_frequency[key] = value / range_01_56
    elif 57 <= key <= 70:
        number_frequency[key] = value / range_57_70
    elif 71 <= key <= 75:
        number_frequency[key] = value / range_70_75

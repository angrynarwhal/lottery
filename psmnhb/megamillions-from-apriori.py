...
#Create a dictionary and whenever one of the newer numbers is called, make the year it was pulled the dictionary key and then append the number to the list that is the value of the key.
#By the end of the loop, we should end up with a dictionary where the keys are the years in which the new numbers have existed, and the values are lists containing every instance one of the new numbers was called that year.
#Then to get the frequency of a new number being called in a given year, we simply count the frequency in which that number is found in the list for that year.
#We can do this across all the years in the dictionary to get the frequency in the years since they have been added, and divide by the total amount of draws in that same time span to get the likelihood of getting one of the new numbers drawn.

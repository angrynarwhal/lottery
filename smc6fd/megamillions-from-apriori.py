...
##################################################################
#Stella Craig's additions

    #find total number of balls
    num_of_balls = data['date'].count()
    # #variable date set to October 22, 2013
    changed_date = '2013-10-22'
    # #find number of balls drawn on or after Oct 22 2013
    num_of_balls_before_change_date = data[data['date'] <= changed_date]['date'].count()
    num_of_balls_after_change_date = num_of_balls-num_of_balls_before_change_date

# #for each ball 57-75
for i in range(57,76):
    new_frequency = round((number_frequency[i] / num_of_balls_after_change_date) * num_of_balls)
    number_frequency[i] = new_frequency

#end of Stella's code :)
##################################################################

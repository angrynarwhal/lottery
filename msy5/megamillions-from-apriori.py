...
switch1Date = pd.Timestamp(2013,10,22)
switch2Date = pd.Timestamp(2017,10,31)
lastDrawing = data['date'].max()
...
for number in number_probabilities.keys():
    if number > 56:
        if number < 71:
            number_probabilities[number] *= weight57_70
        else:
            number_probabilities[number] *= weight71_75

...
# funciton that converts dict into list to get stats on
# returns avg and standard deviation
def getStats(number_dict):
    freq_list = []
    for number, freq in number_dict.items():
        if number > 56:
            break
        freq_list.append(freq)
    
    avg = np.average(freq_list)
    sd = np.std(freq_list)
    return avg, sd
...
# find compare date 10/15/2013
comp_date = data['date'][1191]

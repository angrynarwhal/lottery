...
def normalize_probability(ball, introduction_date):

    current_date = datetime.datetime.now().date()
    draws_since_introduction = (current_date - introduction_date).days
...
data = pd.read_csv('megamillions.csv', parse_dates=[
...
for _, row in data.iterrows():
    ...
    for column in number_columns:
        ...
        if pd.notnull(number):
            number = int(number)
            if 1 <= number <= max_number:  # Check if the number is within the valid range
                cumulative_frequency[number].append(
                    (date, number_frequency[number]))
...
number_probabilities = {
    number: freq / total_draws_1_year for number, freq in number_frequency.items()}

# Sort the numbers by their probability in descending order
sorted_probabilities = sorted(
    number_probabilities.items(), key=lambda x: x[1], reverse=True)
...
with open('most_frequent_numbers.txt', 'w') as f:
    for freq_period in ['W', 'M', 'Q', 'Y']:
        ...
        for number in range(1, max_number + 1):
            date_freq = pd.DataFrame(cumulative_frequency[number], columns=[
                                     'Date', 'Frequency'])
            date_freq.set_index('Date', inplace=True)
            # Convert the index to a datetime-like index
            date_freq.index = pd.to_datetime(date_freq.index)

            if not date_freq.empty:
                date_freq = date_freq.sort_index(ascending=False)
                most_frequent_numbers = date_freq.resample(
                    freq_period)['Frequency'].sum().nlargest(5)
                most_frequent_numbers_by_period.extend(
                    list(zip([number] * len(most_frequent_numbers), most_frequent_numbers)))
        ...
        df = pd.DataFrame(most_frequent_numbers_by_period,

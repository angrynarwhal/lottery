...
date_up_to_75 = pd.to_datetime('10-19-2013', format="%m-%d-%Y")
date_down_to_70 = pd.to_datetime('10-28-2017', format="%m-%d-%Y")

# Iterate through each row of the data and calculate the weekly and cumulative frequencies
for _, row in data.iterrows():
    date = row['date']
    if date > date_up_to_75 and date < date_down_to_70:
...
# Calculate the total number of draws in the acceptable date range
total_draws_valid_period = data[data['date'] >= date_up_to_75]['date'].count() - data[data['date'] >= date_down_to_70]['date'].count()

# Calculate the probability of each number occurring in the acceptable date range
number_probabilities = {number: freq / total_draws_valid_period for number, freq in number_frequency.items()}

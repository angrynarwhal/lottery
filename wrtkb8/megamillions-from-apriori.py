...
data = pd.read_csv('../megamillions.csv', parse_dates=['date'], date_parser=custom_date_parser)
...
# Initialize counts and min/max year values
min_year = data['date'].min()
max_year = data['date'].max()
...
normalization_multiplier = (max_year - min_year) / (CUTOFF_YEAR - min_year)

# Create a new dictionary for normalized probabilities
normalized_probabilities = {}

# Apply the multiplier to probabilities for numbers 57-75
for number, probability in number_probabilities.items():
    if 57 <= number <= 75:
        normalized_probabilities[number] = probability * normalization_multiplier
    else:
        normalized_probabilities[number] = probability

# Sort the numbers by their probability in descending order
sorted_probabilities = sorted(normalized_probabilities.items(), key=lambda x: x[1], reverse=True)
...
plt.tight_layout()
plt.savefig('normalized_probabilities.png')
plt.show()

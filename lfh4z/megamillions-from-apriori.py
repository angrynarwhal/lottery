...
year14=pd.Timestamp("2014-01-01 00:00:00")
dataAfterYear14=data[data['date'] >= year14]
dataBeforeYear14=data[data['date'] < year14]
def getFrequency(data):
    yearNumberFrequency = {number: 0 for number in range(1, max_number + 1)}  # 该#;;#实#=#一年#;;#量#;;#任何#(#备用#够#）
    for _, row in data.iterrows():
        # Calculate the weekly frequency#
        for column in number_columns:
            number = row[column]
            if pd.notnull(number):
                number = int(number)
                if 1 <= number <= max_number:  # Check if the number is within the valid range
                    yearNumberFrequency[number] = yearNumberFrequency.get(number, 0) + 1
    sum1=0
    for i in yearNumberFrequency:
        if i>=57:
            sum1+=yearNumberFrequency[i]
    return yearNumberFrequency

yearNumberFrequency=getFrequency(dataAfterYear14)

sum57To75=0
print(yearNumberFrequency.values())
for i in yearNumberFrequency:
    if i>=57:
        sum57To75+=yearNumberFrequency[i]
print(sum57To75)
print(sum(yearNumberFrequency.values()))
newNumberProportion=sum57To75/(sum(yearNumberFrequency.values())-sum57To75)
print(newNumberProportion)
yearNumberFrequency=getFrequency(dataBeforeYear14)

newNumber=int(sum(yearNumberFrequency.values())*newNumberProportion)
print(newNumber)
randomNum=[]
for i in range(57,75+1):
    randomNum.append(random.randint(20,40))
percentageToAdd=[]

for i in range(57-57,75+1-57):
    percentageToAdd.append(randomNum[i]/sum(randomNum))


listToBeAdded=list(map(lambda x:int(x*newNumber),percentageToAdd))

print(number_frequency)
for i in range(57,75+1):
    number_frequency[i]=number_frequency[i]+listToBeAdded[i-57]
print(number_frequency)
